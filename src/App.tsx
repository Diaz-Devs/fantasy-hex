...
function randomWhisper(list: string[], current?: string): string {
...
function generateFireflies(count = 14): Firefly[] {
...
function App() {
  const [whispers, setWhispers] = useState<string[]>(fallbackWhispers)
  ...
  const [fireflies, setFireflies] = useState<Firefly[]>(() => generateFireflies())
  const [formData, setFormData] = useState({ email: '', password: '' })
  const [authError, setAuthError] = useState('')
  const [authSuccess, setAuthSuccess] = useState('')
  const [user, setUser] = useState<User | null>(null)
  const [activeView, setActiveView] = useState<'landing' | 'login' | 'generator'>('landing')
  ...
  const openLogin = () => setActiveView('login')
  ...
  const renderLanding = () => (
    <>
      ...
      <p className="lede">
        We are weaving cozy strategy, gleaming dice, and a dash of magic into a fresh board game
        adventure. Settle in: the first playable realm is on the way.
      </p>
            ...
            <div className="progress-rows">
              {progressSteps.map((step) => (
                <div className="progress-row" key={step.label}>
                  ...
                  <div className="bar">
                    <span className="fill" style={{ width: `${step.percent}%` }} aria-hidden />
                  </div>
      ...
      <section className="panel login-panel">
        ...
        <form className="login-form" onSubmit={handleLogin}>
          ...
          <div className="form-actions">
            ...
            <div className="subtle helper">This is a local-only check while servers come online.</div>
          </div>
        </form>
      </section>
  ...
  const content = (() => {
    ...
    if (activeView === 'login' && !user) {
      return (
        <>
          <div className="pill">Access portal</div>
          <h1>
            Fantasy Catan access portal
            <span className="accent">Log in to unlock the main menu.</span>
          </h1>
          <p className="lede">
            Sign in to reserve your seat at the table. Once inside you can jump to lobbies, solo
            sessions, and the quickstart guide while the realm keeps growing.
          </p>
          <div className="actions">
            <button className="button ghost" onClick={returnToMain}>
              Back to home
